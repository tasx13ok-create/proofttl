...
export async function handleStudioChat(request, env) {
  ...
  const preference = await loadAccountModelPreference(request, env);
  if (!assistantResponseProviderAvailable(env, preference)) return json({ error: "studio_model_unavailable", message: "The Studio coding model is not configured on this deployment." }, 503);

  const contentType = request.headers.get("content-type") || "";
  if (!contentType.toLowerCase().includes("application/json")) return json({ error: "json_content_type_required", message: "Send application/json." }, 415);

  const limiter = env?.ASSISTANT_RATE_LIMITER;
  if (!limiter || typeof limiter.limit !== "function") return json({ error: "studio_rate_limiter_unavailable", message: "Studio is not configured safely yet." }, 503);

  const ip = (request.headers.get("cf-connecting-ip") || "anonymous").trim().slice(0, 80);
  const limited = await limiter.limit({ key: `studio:${ip}` });
  if (!limited?.success) return json({ error: "studio_rate_limit_exceeded", message: "Too many Studio requests. Try again shortly." }, 429, { "retry-after": "60" });

  let body;
  try { body = await request.json(); }
  catch { return json({ error: "invalid_json", message: "Studio request body must be valid JSON." }, 400); }
  ...
  if (!quota.allowed) return json({ error: "assistant_free_limit_reached", message: "You reached today's ProofTTL AI limit.", quota }, 429, { "retry-after": String(quota.retry_after_seconds) });
  ...
  try {
    const completion = await runAssistantResponse(env, { messages, max_tokens: 700, temperature: 0.25 }, preference);
    ...
    return json({
      ...
      runtime: {
        ...
        preference_applied: runtime.preference_applied,
...
async function loadAccountModelPreference(request, env) {
  if (!env?.MONITOR_DB) return null;
  const session = await getOptionalProofTTLSession(request, env);
  const userId = session?.user?.id;
  if (!userId) return null;
  try {
    const row = await env.MONITOR_DB.prepare("SELECT preferred_ai_provider, preferred_ai_model FROM account_preferences WHERE user_id=?").bind(userId).first();
    return row ? { preferred_ai_provider: row.preferred_ai_provider, preferred_ai_model: row.preferred_ai_model } : null;
  } catch {
    return null;
  }
}
