...
const DEFINITIONS = Object.freeze([
  ...
  { id: 'files.read', area: 'files', risk: 'read', state: 'planned', label: 'Search/read account files' },
  { id: 'files.write', area: 'files', risk: 'modify', state: 'planned', label: 'Create/update account files' },
  { id: 'files.delete', area: 'files', risk: 'sensitive', state: 'planned', label: 'Delete account files' },
  { id: 'automations.manage', area: 'automations', risk: 'sensitive', state: 'planned', label: 'Create recurring/conditional automations' }
]);
...
export function capabilityRegistry(env = {}) {
  ...
  const capabilities = DEFINITIONS.map((item) => ({
    ...item,
    policy: RISK[item.risk],
    ready: capabilityReady(item.id, runtime),
  }));

  return {
    service: 'ProofTTL Capability Registry',
    version: 1,
    principle: 'user_intent_over_app_selection',
    policy: RISK,
    runtime,
    capabilities,
  };
...
function capabilityReady(id, runtime) {
  ...
  if (id === 'studio.projects' || id === 'account.models') return runtime.auth;
...
export function actionPolicy(actionId) {
  ...
  return {
    ...
    risk_label: RISK[capability.risk].label,
    confirmation: RISK[capability.risk].confirmation,
    explicit_confirmation_required: capability.risk === 'sensitive',
  };
}
...
export function planCapabilityAction(input) {
  ...
  if (policy.explicit_confirmation_required && !confirmed) {
    return {
      ...
      policy,
      message: 'Explicit user confirmation is required before this sensitive action may execute.'
    };
  }
  return {
    ok: true,
    ...
    policy,
    message: 'Policy allows the action to proceed to its capability-specific authorization layer.'
  };
}
