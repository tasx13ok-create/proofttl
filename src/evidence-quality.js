...
function independenceAliases(item) {
  const aliases = [];
  if (item.publisher) aliases.push(`p:${item.publisher.toLowerCase()}`);
  try {
    aliases.push(`h:${new URL(item.source_url).hostname.toLowerCase()}`);
  } catch {}
  if (aliases.length === 0 && item.underlying_source_id) aliases.push(`u:${item.underlying_source_id.toLowerCase()}`);
  return aliases;
}

function independentOriginGroups(items) {
  const groups = [];
  items.forEach((item, index) => {
    const aliases = independenceAliases(item);
    if (aliases.length === 0) aliases.push(`unresolved:${index}`);
    const matching = [];
    for (let i = 0; i < groups.length; i += 1) {
      if (aliases.some((alias) => groups[i].aliases.has(alias))) matching.push(i);
    }
    if (matching.length === 0) {
      groups.push({ aliases: new Set(aliases), items: [item] });
      return;
    }
    ...
    for (let i = 0; i < groups.length; i += 1) {
      ...
      for (const alias of group.aliases) mergedAliases.add(alias);
      mergedItems.push(...group.items);
    }
    retained.push({ aliases: mergedAliases, items: mergedItems });
    groups.splice(0, groups.length, ...retained);
  });
  return groups;
}
...
function independentGroupCount(items) {
  return independentOriginGroups(items).length;
}

function sideStrength(items) {
  if (!items.length) return 0;
  const sorted = independentOriginGroups(items)
    .map((group) => Math.max(...group.items.map((item) => item.quality_score)))
    .sort((a, b) => b - a);
