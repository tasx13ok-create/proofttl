...
export async function getDeploymentReadiness(env, request) {
  ...
  const schema = {
    ...
    account_files: await tableExists(env?.MONITOR_DB, "account_files")
  };
  ...
  const accountChecks = {
    ...
    account_files_schema: schema.account_files,
  ...
  const workspaceChecks = {
    ...
    native_files: schema.account_files,
  ...
  if (!schema.account_files) productionBlockers.push("account_files_schema");
  ...
  return {
    ...
    workspace: {
      ...
      native_account_files: schema.account_files,
    ...
    note: "Readiness is split by product surface so testnet protocol, commercial audit services, customer accounts, universal Workspace, AI, Files, and Studio can be evaluated truthfully without implying unfinished providers or mainnet settlement are live."
  };
}
