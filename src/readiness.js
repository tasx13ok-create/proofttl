...
export async function getDeploymentReadiness(env, request) {
  ...
  return {
    ...
    commercial_services:{ready:Object.values(commercialChecks).every(Boolean),payment_provider:"stripe",payment_mode:commercialChecks.stripe_live_mode?"live":"test_or_unconfigured",offers:["fact_audit_1500"],scope_before_payment:true,checks:commercialChecks,note:"Human-facing audit services are commercially ready only when live Stripe credentials, webhook verification, D1 sales schema, the public web URL, and the admin control token are all configured."},
