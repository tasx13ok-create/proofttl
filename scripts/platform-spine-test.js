...
const accountFiles = read('src/account-files.js');
const accountTasks = read('src/account-tasks.js');
const assistantQuota = read('src/assistant-quota.js');
const ownerAccess = read('src/owner-access.js');
...
const checks = [
  ...
  [ownerAccess.includes('isProofTTLOwnerSession') && ownerAccess.includes('foundryAccessAllowed'), 'central owner-access policy is reusable across product surfaces'],
  [assistantQuota.includes('owner_unmetered') && assistantQuota.includes('unlimited: true'), 'owner assistant usage is application-level unmetered'],
  [accountFiles.includes('unlimited_file_count') && accountFiles.includes('if (!owner)'), 'owner native file count bypasses ordinary account quota'],
  [accountFiles.includes('MAX_FILE_BYTES') && accountFiles.includes('file_too_large'), 'owner file quota bypass does not remove payload safety limit'],
  [accountTasks.includes('unlimited_task_count') && accountTasks.includes('if (!owner)'), 'owner task count bypasses ordinary account quota'],
];
